...
for i=2:length(t)
    ...
    %over the course of 1 second (100 bc .01 dt)neurons voltage increase by
    %5mV (which was divided /100)
...
injectedCurrent=15*dt; %The neuron's voltage will increase by 5mV/sec
neuronVoltage(1)=restingV; %setting the first value to resting potential
for i=1:length(t)-1%execute for time minus one 
    if  neuronVoltage(i)<threshold && injectedCurrent>0;
         
    neuronVoltage(i+1)=neuronVoltage(i)+injectedCurrent;
   injectedCurrent=15*dt;
    end
    %over the course of 1 second (100 bc .01 dt)neurons voltage increase by
    %5mV (which was divided /100)
    ...
    if neuronVoltage(i)>=threshold && injectedCurrent~=0  %If reached threshold, make neuron spike
        neuronVoltage(i)=50;%the spike (the overshoot)
        injectedCurrent=0;
        disp('spike')
    end
    if injectedCurrent==0
        neuronVoltage(i+1)=neuronVoltage(i)+(restingV-neuronVoltage(i))*dt;
   injectedCurrent=0;
    end
    if neuronVoltage(i)<restingV && injectedCurrent==0
        injectedCurrent=15*dt;
        neuronVoltage(i+1)=neuronVoltage(i);
